...
ORDENAR CONSULTA
    ...
    !!!!!!!!!!!!!!!!
    EJERCICIO
    MOSTRAR LOS ALUMNOS QUE CUMPLEN AÑOS EN DICIEMBRE Y ORDENAR DE MENOR A MAYOR
    MONTH(fechanacimiento)

SELECT * FROM alumnos WHERE MONTH(fechanacimiento)=12 ORDER BY fechanacimiento DESC;
...
FUNCIONES AGREGADORAS
    ...
    SELECT YEAR(fechanacimiento), COUNT(id) FROM alumnos GROUP BY YEAR(fechanacimiento) ORDER BY YEAR(fechanacimiento) DESC;
...
COMO BUSCAR EN UNA BASE DE DATOS
    ...
    SQL NO DISTINGUE MAYUSCULAS DE MINUSCULAS
    ...
    MOSTRAR LOS PROFESORES QUE SON 
    ...
    LOS QUE SON ESPECIALISTAS EN PSICOLOGIA Y BIOLOGIA SOLAMENTE

    PROFESORES CUYO APELLIDO ACABA EN EZ
    
     SELECT * FROM profesores WHERE apellidos LIKE "%ez";
...
MULTIPLES CONDICIONES
    ...
    !!!!!!!!!!!!!!!!
    SELECCIONAR LOS ALUMNOS QUE SON DEL 2002 Y QUE SON DEL CURSO CON ID 10

    SELECT * FROM alumnos WHERE YEAR(fechanacimiento)=2002 AND idcurso=10;

    SELECCIONAR ALUMNOS CON ID 10,8,7

    SELECT * FROM alumnos WHERE idcurso IN(10,8,7);
